...
Run the following command to start Next.js, Inngest dev server, and Prisma Studio simultaneously:
...
- Inngest Dev Server at [http://localhost:8288](http://localhost:8288)
...
3. In another terminal, start Prisma Studio (optional, for database management):
```bash
npm run dev:prisma
```
...
**Once deployed, make sure to configure your AI API keys:**
- For direct OpenAI access: Set `OPENAI_API_KEY` environment variable
- For LiteLLM proxy: Set `LITELLM_API_KEY` and `LITELLM_BASE_URL` environment variables
