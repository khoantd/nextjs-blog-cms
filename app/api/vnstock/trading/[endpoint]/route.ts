...
/**
 * Proxy requests to vnstock trading endpoints
...
export async function POST(
  ...
  try {
    ...
    const targetUrl = `${VNSTOCK_API_URL}/api/v1/trading/${endpoint}`;
    ...
    try {
      ...
      console.error('Vnstock trading API network error:', fetchError);
      return NextResponse.json(
        { error: errorMessage },
        { status: 503 }
      );
    ...
    console.error('Vnstock trading API error:', error);
    return NextResponse.json(
      { error: error.message || 'Failed to fetch trading data' },
      { status: 500 }
    );
  }
}
